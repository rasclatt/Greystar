...
use \Greystar\Dto\Reports\GetReport\Request as GetReportDto;
...
class Reports extends \Greystar\Model
{
    ...
    public function getReport(
        GetReportDto $request,
        Callable $func = null,
        Callable $override = null
    )
    {
        $anon = [];
        $html = $this->getDataTable($request->reportcode, array_filter($request->toArray()));
        if (empty($override)) {
            ...
            return ($func) ? $func(array_filter($anon)) : array_filter($anon);
        } else
